...
def get_vacancies_by_salary(filtered_vacancies, min_salary):
    ...
    filtered_vacancies_twoo = [vacancy for vacancy in filtered_vacancies if
                          vacancy.salary_from and vacancy.salary_from >= min_salary]
...
def print_top_vacancies(filtered_vacancies, top_n):    #переименовать filtered_vacancies в лист
    if len(filtered_vacancies) < top_n:
        print(f"MALO EST TOKO {len(filtered_vacancies)}")
    else:
